document.addEventListener('DOMContentLoaded', () => {
    ...
    form.addEventListener('submit', async (event) => {
        ...
        const urlCadastro = 'http://localhost:5500/api/auth/register'; 
        ...
        try {
            ...
            if (response.ok) { // Verifica se o status é 2xx (Sucesso)
                ...
                mensagemElement.textContent = data.mensagem || 'Cadastro realizado com sucesso!';
                mensagemElement.style.color = 'green';
                form.reset(); // Limpa o formulário após sucesso
                window.location.href = "../index.html"
            } else {
                // Se o status for 400, 500, etc. (Erro)
                mensagemElement.textContent = `Erro no cadastro: ${data.erro || 'Tente novamente.'}`;
